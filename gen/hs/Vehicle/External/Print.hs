...
instance Print Vehicle.External.Abs.Boolean where
  prt _ (Vehicle.External.Abs.Boolean (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.TokNetwork where
  prt _ (Vehicle.External.Abs.TokNetwork (_,i)) = doc $ showString (Data.Text.unpack i)
instance Print Vehicle.External.Abs.TokDataset where
  prt _ (Vehicle.External.Abs.TokDataset (_,i)) = doc $ showString (Data.Text.unpack i)
instance Print Vehicle.External.Abs.TokParameter where
  prt _ (Vehicle.External.Abs.TokParameter (_,i)) = doc $ showString (Data.Text.unpack i)
instance Print Vehicle.External.Abs.TokProperty where
  prt _ (Vehicle.External.Abs.TokProperty (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.TokUnit where
  prt _ (Vehicle.External.Abs.TokUnit (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.TokVector where
  prt _ (Vehicle.External.Abs.TokVector (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.TokIndex where
  prt _ (Vehicle.External.Abs.TokIndex (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.TokForeach where
  prt _ (Vehicle.External.Abs.TokForeach (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.TokNil where
  prt _ (Vehicle.External.Abs.TokNil (_,i)) = doc $ showString (Data.Text.unpack i)
...
instance Print Vehicle.External.Abs.DeclAnnOptionValue where
  prt _ (Vehicle.External.Abs.DeclAnnOptionValue (_,i)) = doc $ showString (Data.Text.unpack i)
instance Print [Vehicle.External.Abs.Name] where
  prt _ [] = concatD []
...
instance Print Vehicle.External.Abs.Lit where
  prt i = \case
    Vehicle.External.Abs.UnitLiteral -> prPrec i 0 (concatD [doc (showString "()")])
    Vehicle.External.Abs.NatLiteral natural -> prPrec i 0 (concatD [prt 0 natural])
    Vehicle.External.Abs.RatLiteral rational -> prPrec i 0 (concatD [prt 0 rational])
    Vehicle.External.Abs.BoolLiteral boolean -> prPrec i 0 (concatD [prt 0 boolean])
...
instance Print Vehicle.External.Abs.Expr where
  prt i = \case
    ...
    Vehicle.External.Abs.Foreach tokforeach binders tokdot expr -> prPrec i 2 (concatD [prt 0 tokforeach, prt 0 binders, prt 0 tokdot, prt 2 expr])
    ...
    Vehicle.External.Abs.VecLiteral tokseqopen exprs tokseqclose -> prPrec i 15 (concatD [prt 0 tokseqopen, prt 0 exprs, prt 0 tokseqclose])
    Vehicle.External.Abs.Nil toknil -> prPrec i 15 (concatD [prt 0 toknil])
    ...
    Vehicle.External.Abs.Unit tokunit -> prPrec i 15 (concatD [prt 0 tokunit])
    ...
    Vehicle.External.Abs.Vector tokvector expr1 expr2 -> prPrec i 14 (concatD [prt 0 tokvector, prt 15 expr1, prt 15 expr2])
    Vehicle.External.Abs.Tensor toktensor expr1 expr2 -> prPrec i 14 (concatD [prt 0 toktensor, prt 15 expr1, prt 15 expr2])
    Vehicle.External.Abs.Index tokindex expr -> prPrec i 14 (concatD [prt 0 tokindex, prt 15 expr])
...
instance Print Vehicle.External.Abs.DeclAnnName where
  prt i = \case
    Vehicle.External.Abs.Network toknetwork -> prPrec i 0 (concatD [prt 0 toknetwork])
    Vehicle.External.Abs.Dataset tokdataset -> prPrec i 0 (concatD [prt 0 tokdataset])
    Vehicle.External.Abs.Parameter tokparameter -> prPrec i 0 (concatD [prt 0 tokparameter])
    Vehicle.External.Abs.Property tokproperty -> prPrec i 0 (concatD [prt 0 tokproperty])

instance Print Vehicle.External.Abs.DeclAnnOption where
  prt i = \case
    Vehicle.External.Abs.BooleanOption name boolean -> prPrec i 0 (concatD [prt 0 name, doc (showString "="), prt 0 boolean])
...
instance Print Vehicle.External.Abs.DeclAnnOpts where
  prt i = \case
    Vehicle.External.Abs.DeclAnnWithOpts declannoptions -> prPrec i 0 (concatD [doc (showString "("), prt 0 declannoptions, doc (showString ")")])
    Vehicle.External.Abs.DeclAnnWithoutOpts -> prPrec i 0 (concatD [])

instance Print Vehicle.External.Abs.Decl where
  prt i = \case
    Vehicle.External.Abs.DefAnn declannname declannopts -> prPrec i 0 (concatD [prt 0 declannname, prt 0 declannopts])
