...
delabNameBinder b = case V.binderNamingForm b of
  V.OnlyType{}    -> developerError $ "Should not be delaborating an `OnlyType` binder of type" <+>
                      pretty (show (V.binderRepresentation b)) <+> "to a `NamedBinder`"
...
delabTypeBinder b = case V.binderNamingForm b of
  V.OnlyName{}    -> developerError $ "Should not be delaborating an `OnlyName` binder of type" <+>
                      pretty (show (V.binderRepresentation b)) <+> "to a `TypeBinder`"
...
delabOp2 op tk = \case
  ...
  args | length args > 2 -> argsError (tkSymbol tk) 2 args
...
delabOp3 op tk = \case
  ...
  args | length args > 3 -> argsError (tkSymbol tk) 3 args
