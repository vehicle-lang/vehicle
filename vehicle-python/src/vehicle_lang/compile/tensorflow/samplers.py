...
class TensorFlowSampler(ABCSampler[int, tf.Tensor]):
    @abstractmethod
    def get_loss(
            self,
            dims: int,
...
class DefaultTensorFlowSampler(TensorFlowSampler):
    ...
    def __init__(self, num_samples: int = 10, num_steps: int = 5, seed: int | None = None):
        """
    ...
    def get_loss(
        ...
        for _ in range(self.num_samples):
            ...
            current_point = tf.add(lb, tf.multiply(
            ...
            for _ in range(self.num_steps):
                ...
                if gradient is None:
                    ...
                    gradient = tf.where(tf.math.is_nan(gradient), tf.zeros_like(gradient), gradient)
