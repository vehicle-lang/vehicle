...
as dataclasses, prints the controller's output on a sample Input, then runs
Adam over the safe property's loss.

Run from this directory:

    uv run python train.py

The dataclass types are obtained from 'spec[<name>]' at runtime, so mypy
sees them as Any. Field access works correctly at runtime; static checks
can't verify it.
"""
...
def make_controller(Output: type, hidden: int = 16) -> nn.Module:
    """Build an MLP whose forward takes an Input dataclass and returns an Output."""
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    controller = make_controller(Output)
