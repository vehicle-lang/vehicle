...
theory  WindControllerSpecOptimal
...
begin
  ...
  definition normalise  :: " (InputVector \<Rightarrow> OutputVector) \<Rightarrow> (InputVector) \<Rightarrow>  InputVector "
    where " normalise   controller x = Abs_InputVector ( (foreach (2 :: nat)  (\<lambda>  i . ((pointwise_div ((subtensor_lookup x i) + (tensor_from_vec [1] [ (((4 :: R) )) ])) (tensor_from_vec [1] [ (((8 :: R) )) ]))))) ) "
  ...
  definition safeOutput  :: " (InputVector \<Rightarrow> OutputVector) \<Rightarrow> (InputVector) \<Rightarrow>  bool "
    where " safeOutput   controller x = ( let y = (subtensor_lookup ((controller ((normalise controller x)))) velocity) in (ltTensorReduced ((-1 :: R) \<cdot> (tensor_from_vec [1] [ (((5 :: R) / 4)) ])) ((((y + ((hadamard_prod (tensor_from_vec [1] [ (((2 :: R) )) ]) (subtensor_lookup x currentSensor))))) + ((-1 :: R) \<cdot> (subtensor_lookup x previousSensor))))) \<and> (ltTensorReduced ((((y + ((hadamard_prod (tensor_from_vec [1] [ (((2 :: R) )) ]) (subtensor_lookup x currentSensor))))) + ((-1 :: R) \<cdot> (subtensor_lookup x previousSensor)))) (tensor_from_vec [1] [ (((5 :: R) / 4)) ])) ) "
