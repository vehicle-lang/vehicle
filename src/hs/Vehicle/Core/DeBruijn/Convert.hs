...
data DeBruijnConversionError = FreeVariableError Name
...
instance DBConvert (Kind SortedName builtin ann) (Kind SortedDeBruijn builtin ann) where
...
instance DBConvert (Type SortedName builtin ann) (Type SortedDeBruijn builtin ann) where
  ...
  convert ctxt (TVar ann (SortedName name)) = do
    index <- convertName name ctxt
    return $ TVar ann (SortedDeBruijn index)

instance DBConvert (Expr SortedName builtin ann) (Expr SortedDeBruijn builtin ann) where
  ...
  convert ctxt (EVar ann (SortedName name)) = do
    index <- convertName name ctxt
...
instance DBConvert (Decl SortedName builtin ann) (Decl SortedDeBruijn builtin ann) where
...
convertEArg :: EArg SortedName builtin ann -> (Text , EArg SortedDeBruijn builtin ann)
convertEArg (EArg ann (SortedName name)) = (getText name , EArg ann (SortedDeBruijn name))

convertTArg :: TArg SortedName builtin ann -> (Text , TArg SortedDeBruijn builtin ann)
convertTArg (TArg ann (SortedName name)) = (getText name , TArg ann (SortedDeBruijn name))

convertTArgs :: MonadDBConvert m => [Text] -> [TArg SortedName builtin ann] -> m ([Text], [TArg SortedDeBruijn builtin ann])
