...
data PropertyState a
  = Trivial Bool
  | NonTrivial a
  deriving (Generic)

instance ToJSON a => ToJSON (PropertyState a)
instance FromJSON a => FromJSON (PropertyState a)

instance Functor PropertyState where
  fmap f = \case
    Trivial s    -> Trivial s
    NonTrivial s -> NonTrivial (f s)

traversePropertyState :: Monad m => PropertyState a -> (a -> m b) -> m (PropertyState b)
traversePropertyState s f = case s of
  Trivial b    -> return $ Trivial b
  NonTrivial a -> NonTrivial <$> f a

disjunctPropertyStates :: [PropertyState a] -> PropertyState (NonEmpty a)
disjunctPropertyStates = flip foldr (Trivial False) $ \p1 p2 -> case p1 of
  Trivial True  -> Trivial True
  Trivial False -> p2
  NonTrivial s1 -> case p2 of
    Trivial True  -> Trivial True
    Trivial False -> NonTrivial (s1 :| [])
    NonTrivial s2 -> NonTrivial (s1 <| s2)

conjunctPropertyStates :: [PropertyState a] -> PropertyState (NonEmpty a)
conjunctPropertyStates = flip foldr (Trivial True) $ \p1 p2 -> case p1 of
  Trivial True  -> p2
  Trivial False -> Trivial False
  NonTrivial s1 -> case p2 of
    Trivial True  -> NonTrivial (s1 :| [])
    Trivial False -> Trivial False
    NonTrivial s2 -> NonTrivial (s1 <| s2)

instance Pretty a => Pretty (PropertyState a) where
  pretty = \case
