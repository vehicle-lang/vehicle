  ...
  showEntry expr $ do
    ...
    case toRatTensorValue forcedValue of
      ...
      VRatTensorBoundVar v -> do
        expanded <- unblockRatTensorBoundVar v
        forcedExpansion <- forceThunk expanded
        case forcedExpansion of
          VBoundVar v' [] | v' == v -> return $ IfLeaf expr
          _ -> unblock expanded
...
unblockTranspose ::
  (MonadUnblock m) =>
  TypeUnblockingFunction (Thunk Builtin) m ->
  OperationUnblockingFunction TransposeArgs (Thunk Builtin) m
unblockTranspose unblock (TransposeArgs t ds xs) = do
  xs' <- unblock xs
  forIfTreeM xs' $ \xs'' ->
    IfLeaf <$> forceEvaluation accessTranspose evalTranspose (TransposeArgs t ds xs'')
...
unblockAtTensor unblock unblockTensor unblockIndex (AtTensorArgs tElem d ds xs i) = do
  ...
  forIfTreeM xs' $ \xs'' ->
    forIfTreeM i' $ \i'' -> do
      let args = AtTensorArgs tElem d ds xs'' i''
      result <- evalAtTensor args
      case result of
        Evaluated reduced -> unblock reduced
        Unevaluable {} -> return $ IfLeaf $ Forced $ mkExpr accessAtTensor args
