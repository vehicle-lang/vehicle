...
runTypeCheckerTInitially importedCtx e = do
  let state = emptyTypeCheckerState importedCtx
...
runConstraintSolver ::
  ...
  (MonadTypeChecker builtin m, NormalisableBuiltin builtin, PrettyExternal (Contextualised constraint (ConstraintContext builtin))) =>
