...
class (TypableBuiltin builtin) => HasTypeSystem builtin where
  -- | Used to match `@builtin` declarations
  fromBuiltinName :: Name -> Maybe builtin
...
lookupBuiltin :: (HasTypeSystem builtin) => Name -> builtin
lookupBuiltin name = case fromBuiltinName name of
  Nothing -> developerError ("Unknown @builtin" <+> quotePretty name)
