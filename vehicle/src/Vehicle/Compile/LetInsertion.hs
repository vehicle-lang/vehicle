...
instance Semigroup a => Semigroup (GenericSubexpression a) where
  (CSItem e1 m ps) <> (CSItem e2 n qs)
    ...
    | otherwise =
        developerError $
          "Merging non-identical subexpressions during let lifting:"
            <> line
...
liftOverBinder ::
  MonadLetInsert m =>
  (CoDBExpr, SubexpressionMap) ->
  m (CoDBExpr, SubexpressionMap)
...
letBindSubexpressions remainingSM subexprsToInsert expr
  ...
  where
    ...
    updateSubexpression p1 (CSItem v2 q p2) =
      let (remainder, suffix) = stripPrefix p1 p2
       in -- The position (if any) in the subexpression bound in the newly inserted let
          let letPosition = listToMaybe suffix
           in -- The updated positions in body of the newly inserted let
              let bodyPositions = There . Here <$> remainder
               in case mergeBoth letPosition bodyPositions of
...
nodeSM :: CoDBExpr -> [SubexpressionMap] -> SubexpressionMap
nodeSM e sms =
  -- Merge the maps together
  let mergedCSIMap = fmap (fmap Node) (foldr merge Map.empty $ fmap (fmap (fmap Here)) sms)
   in -- Add the current node to the map
      let eHash = hash e
       in let item = CSItem e 1 Leaf
           in Map.insertWith (duplicateError e sms) eHash item mergedCSIMap
  where
    merge ::
...
duplicateError ::
  ...
  Subexpression ->
  Subexpression
duplicateError e maps _ _ =
  developerError $
    "During let-lifting found duplicate sub-expression"
...
prettyItem ::
  ...
  Subexpression ->
  Maybe (Doc a)
prettyItem expr subexprFilter item =
  if filterItem subexprFilter item
    then Just $ pretty (quantity item) <+> "of" <+> prettyEntry expr item
    else Nothing

prettySM :: CoDBExpr -> (CoDBExpr -> Int -> Bool) -> SubexpressionMap -> Doc a
prettySM expr subexprFilter sm =
  pretty (layoutAsText <$> mapMaybe (prettyItem expr subexprFilter) (Map.elems sm))

prettyCommonSubExprs :: CoDBExpr -> [Subexpression] -> Doc a
