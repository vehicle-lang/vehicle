...
partition var = foldr categorise ([], [], [])
  where
    ...
    categorise a@(Assertion rel (LinearExpr expr)) (less, greater, unused) =
      let coeff = expr Vector.! var
       in if coeff < 0
            then
              let coeff' = -coeff
               in let expr' = Vector.map (/ coeff') expr
                   in let a' = Assertion rel (LinearExpr expr')
                       in (a' : less, greater, unused)
            else
              if coeff > 0
                then
                  let expr' = Vector.map (/ coeff) expr
                   in let a' = Assertion rel (LinearExpr expr')
                       in (less, a' : greater, unused)
...
reconstructFourierMotzkinVariableValue assignment solution = do
  ...
  where
    evaluateMinValue :: Assertion -> (Double, Relation) -> (Double, Relation)
    ...
    evaluateMaxValue :: Assertion -> (Double, Relation) -> (Double, Relation)
