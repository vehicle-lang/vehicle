...
fourierMotzkinElimination ::
  (MonadCompile m, Constant constant) =>
  Variable ->
  [Inequality constant] ->
  m (Bounds constant, [Inequality constant])
...
partition :: (Constant constant) => Variable -> [Inequality constant] -> (Bounds constant, [Inequality constant])
