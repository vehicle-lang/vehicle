    ...
    Right contents -> case decode contents of
      ...
      Just actualInfo -> do
        ...
        if actualVersion == expectedVersion
          then do
            logDebug MaxDetail $ "Found up-to-date installed version of" <+> quotePretty name <+>
              "at" <+> quotePretty libraryFolder
            return True
          else do
            logDebug MaxDetail $ "Installed version of" <+> quotePretty name <+>
              parens (pretty actualVersion) <+> "does not match latest version" <+>
