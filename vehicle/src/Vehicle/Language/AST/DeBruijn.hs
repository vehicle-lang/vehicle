...
liftDBIndices :: Substitutable value target
              => (Provenance -> DBVar -> value) -- ^ function to make a variable
              -> Int                            -- ^ amount to lift by
              -> target                         -- ^ target term to lift
              -> target                         -- ^ lifted term
liftDBIndices mkVar d = substitute (\p v -> Just $ mkVar p (Bound (v+d)))

-- | De Bruijn aware substitution of one expression into another
substIntoAtLevel :: Substitutable value target
...
substIntoAtLevel level value =
  substitute (\_p v -> if v == level then Just value else Nothing)
