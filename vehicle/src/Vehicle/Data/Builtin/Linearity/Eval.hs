...
evalBuiltinFunction :: (MonadLogger m) => BuiltinFunction -> EvalBuiltin (Value closure LinearityBuiltin) m
evalBuiltinFunction b evalApp originalValue args = case b of
  ...
  where
    ...
    notImplemented = developerError $ "Normalisation of " <+> pretty b <+> "at the type-level not yet supported for Linearity system"
