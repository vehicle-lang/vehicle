...
import Vehicle.Data.Variable.Bound.Context.Instance (BoundContext, BoundContextT, runBoundContext, runBoundContextT, runFreshBoundContext, runFreshBoundContextT)
...
addNamesToContext :: (MonadNameContext m) => [Name] -> m a -> m a
addNamesToContext names = addBindersToContext (fmap (\n -> mkExplicitBinder () (Just n)) names)
