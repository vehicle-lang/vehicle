          ...
          PrefixTestCase
            { _testName = "prefix: ((x+y)/(x+y))+y || x+y && y",
              ...
              tree2 =
                Node
                  ( Both
                      (Node (Both (Node (There (Here Leaf))) (Here (Node (There (Here Leaf))))))
                      (Here Leaf)
                  ),
              remainder = Just (Node (There (Here Leaf))),
...
prefixTest (PrefixTestCase testName tree1 tree2 expectedRemainder expectedSuffixes) =
  ...
  where
    ...
    errorMessage =
      "Expected prefixes to be equal to "
        <> show expectedResult
        <> " but was actually "
        <> show result
        <> " and the reverse application"
        <> " resulted in "
        <> show revResult
...
substPosTest (SubstPosTestCase testName valueText positions exprText expectedResult) =
  unitTestCase testName $ do
    ...
    let errorMessage =
          layoutAsString $
            "Expected the result of substituting"
              <+> squotes (pretty valueText)
              <+> "at positions"
                <> line
                <> indent 2 (squotes (pretty positions))
                <> line
                <> "into"
                <> line
