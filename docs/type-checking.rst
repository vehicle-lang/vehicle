...
- operations are applied to the right types of data
  (e.g. we do not add a ``Vector`` to a ``Bool``).
- variables are used consistently
  (e.g. the same variable ``x`` isn't used as an ``Nat`` in one place and
  as a ``Tensor`` in another).
- there are no out-of-bounds errors when indexing into ``Vector`` and
  ``Tensor``
  (e.g. if ``xs`` is a vector of size 2 then we don't index into it
