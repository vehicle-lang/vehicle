...
the ``vehicle compile queries`` command, e.g.

.. code-block:: bash

  vehicle compile queries \
    --format MarabouQueries
...
.. option:: --format, -t
    ...
    The compilation format. There is currently two query formats supported:
    ``MarabouQueries`` and ``VNNLibQueries``.
