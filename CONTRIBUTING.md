...
<a id="installing-ghc-and-cabal"></a>
...
- GHC and Cabal—specifically [our preferred version of GHC](#the-preferred-version-of-ghc)
...
`uv sync` reads `vehicle-python/pyproject.toml`, resolves dependencies into `uv.lock`, and materialises the environment in `.venv`. The base dependency set is intentionally small; install whichever extras you need with the `--extra` flag (see [Managing dependencies](https://docs.astral.sh/uv/concepts/projects/dependencies/)). PyTorch is the recommended backend, so most workflows begin with:
...
Use pytest selectors such as `-k` to narrow to specific names, _e.g._:
...
This creates the directory `dist` which contains "wheels", which are the binary distribution format for Python packages. These wheels will have file names such as `vehicle_lang-0.26.0-cp311-cp311-macosx_13_0_arm64`:
...
#   versions      \
#                  vvvvvvvvvvv
vehicle_lang-0.26.0-cp311-cp311-macosx_13_0_arm64
#                              ^^^^^^^^^^^^^^^^^
#   Supported                /
...
Ensure that you have the source code and that you have installed both GHC and Cabal and a supported Python interpreter plus [`pipx`][pipx].
...
9. **On a macOS machine with an M1/M2 chipset**
  ...
  ```
   ...
   ```
   vehicle_lang-0.26.0-cp310-cp310-macosx_13_0_arm64.whl
   vehicle_lang-0.26.0-cp37-cp37m-macosx_13_0_arm64.whl
   vehicle_lang-0.26.0-cp39-cp39-macosx_13_0_arm64.whl
   vehicle_lang-0.26.0-cp311-cp311-macosx_13_0_arm64.whl
   vehicle_lang-0.26.0-cp38-cp38-macosx_13_0_arm64.whl
   ```
   ...
   **Warning**: The following is a destructive action! Published versions cannot be changed!
   ...
   <https://github.com/vehicle-lang/vehicle/releases/tag/v0.26.0>
